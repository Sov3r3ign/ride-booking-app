...
async function showRideRequest(rideId) {
  ...
  try {
    ...
    // Set ride type
    const rideTypeLabels = { ride: 'Ride', rideL: 'RideL', rideXL: 'RideXL' };
    document.getElementById('request-ride-type').textContent = rideTypeLabels[ride.rideType] || 'Ride';
...
function createRideCard(ride) {
  ...
  const coordsDisplay = `
    <div class="coords">
      <strong> Rider</strong> ${ride.riderUsername || 'Unknown'}<br>
      <strong> Ride Type</strong> ${rideTypeLabel} (${ride.seats || 2} seats)<br>
      <strong> Pickup</strong><br>
      <span id="card-pickup-${ride._id}">Loading location...</span><br>
      <br>
      <strong> Dropoff</strong><br>
      <span id="card-dropoff-${ride._id}">Loading location...</span><br>
      <br>
      <strong> Distance</strong> ${ride.distance != null ? ride.distance.toFixed(2) + ' km' : 'N/A'}<br>
      <strong> Fare</strong> ${ride.fare != null ? 'R' + ride.fare.toFixed(2) : 'N/A'}<br>
      <br>
