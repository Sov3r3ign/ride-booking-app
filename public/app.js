...
// The rider's current pending/accepted ride, if any. While this is set,
// the rider can't start booking a new one -- they first have to cancel it
// (if still pending) or wait for the driver to complete it.
let activeRide = null;
...
// While the rider has an active ride, periodically check its status so we
// notice (without a page refresh) when the driver completes it -- or when
// it stops being active for any other reason -- and unlock booking again.
  ...
  activeRidePollInterval = setInterval(async () => {
    ...
    try {
      ...
      if (ride.status === 'pending' || ride.status === 'accepted') {
        ...
        // Completed or cancelled elsewhere (e.g. the driver finished it) --
        // unlock booking.
        activeRide = null;
        stopActiveRidePolling();
        showCurrentRide(ride);
        ...
        updateInstruction(
          ride.status === 'completed'
            ? ' Ride completed! Click the map to request another.'
...
requestBtn.addEventListener('click', async function () {
  ...
  try {
    ...
    if (response.status === 409) {
      // Another tab (or a stale UI state) already has an active ride --
      // sync up to it instead of erroring out blindly.
    ...
    setTimeout(() => {
      resetMarkers();
      updateInstruction(' Ride requested! Track its status in the panel below.', 'success');
    }, 1500);
...
document.addEventListener('DOMContentLoaded', async () => {
  // Restore any ride the rider already had in flight (e.g. after a page
  // refresh) so a reload can't be used to slip past the one-active-ride
  // rule -- and so they can see/cancel it right away instead of it being
  // silently forgotten.
  try {
...
// Mirrors the server's fare formula
const BASE_FARE = 25;
const RATE_PER_KM = 8;
const RIDE_TYPE_MULTIPLIERS = { ride: 1, rideL: 1.4, rideXL: 1.8 };
...
function showCurrentRide(ride) {
  ...
  // Show "Request Another" button for completed rides
  const requestAnotherHtml = (ride.status === 'completed')
